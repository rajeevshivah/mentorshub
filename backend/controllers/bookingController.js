...
exports.createBooking = async (req, res) => {
  try {
    ...
    if (status === "confirmed") {
      sendBookingConfirmation(studentInfo.email, booking)
        .then(() => console.log("✅ Confirmation email sent"))
        .catch(err => console.error("❌ Email error:", err.message));
    } else {
      sendUpiPendingEmail(studentInfo.email, booking)
        .then(() => console.log("✅ UPI pending email sent"))
        .catch(err => console.error("❌ Email error:", err.message));
    }
...
exports.confirmUpiBooking = async (req, res) => {
  try {
    ...
    // Send email in background — don't wait for it
    sendUpiConfirmedEmail(booking.studentInfo.email, booking)
      .then(() => console.log("✅ UPI confirmed email sent"))
      .catch(err => console.error("❌ Email error:", err.message));
